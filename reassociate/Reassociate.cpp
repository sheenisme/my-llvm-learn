...
static BinaryOperator *LowerNegateToMultiply(Instruction *Neg)
{
    ...
    Type *Ty = Neg->getType();
    Constant *NegOne = Ty->isIntOrIntVectorTy() ? ConstantInt::getAllOnesValue(Ty) : ConstantFP::get(Ty, -1.0);
        ...
        if (BinaryOperator *BO = isReassociableOp(Op, Opcode)) {
          ...
          continue;
            ...
            assert((!isa<Instruction>(Op) ||
                    cast<Instruction>(Op)->getOpcode() != Opcode || (isa<FPMathOperator>(Op) && !cast<Instruction>(Op)->isFast())) &&
                          ...
                          ReassociatePass::OrderedSet &ToRedo)
{
    if (auto *C = dyn_cast<Constant>(V))
        return C->getType()->isFPOrFPVectorTy() ? ConstantExpr::getFNeg(C) : ConstantExpr::getNeg(C);
                               ...
                               SmallVectorImpl<ValueEntry> &Ops)
{
    ...
    for (unsigned i = 0, e = Ops.size(); i != e; ++i)
    {
        ...
        assert(i < Ops.size());
        Value *X;
        if (match(Ops[i].Op, m_Not(m_Value(X))))
        { // Cannot occur for ^.
                                    ...
                                    SmallVectorImpl<ValueEntry> &Ops)
{
    ...
    for (unsigned i = 0, e = Ops.size(); i != e; ++i)
    {
        ...
        if (i + 1 != Ops.size() && Ops[i + 1].Op == TheOp)
        {
            ...
            Constant *C = Ty->isIntOrIntVectorTy() ? ConstantInt::get(Ty, NumFound) : ConstantFP::get(Ty, NumFound);
...
void ReassociatePass::BuildPairMap(ReversePostOrderTraversal<Function *> &RPOT)
{
